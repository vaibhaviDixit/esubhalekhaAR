...
if(isset($_REQUEST['cardID'])){
    ...
    $cardData = DB::select('smartCard ', " * ", " cardID = '$cardID' ")->fetch();
    DB::close();

    $btn = "Update";
	$productGallery = $gallery->getProductGallery($_REQUEST['cardID']);
  ...
  // delete img by url
    if(isset($_REQUEST['imgurl'])){
        ...
        $gallery=new Gallery();
        $getrow=$gallery->deleteProductByURL($_REQUEST['cardID'],$imgurl);
	...
	<form method="post" id="form" class="form-wedding" enctype="multipart/form-data">
			<?php
				if (isset($_REQUEST['btn-submit'])) {
					...
					if(isset($_REQUEST['cardID'])){
						$createProduct = $smartCard->update($_REQUEST['cardID'], $_REQUEST);	
					}else{
						$createProduct = $smartCard->create($_REQUEST['name'], $_REQUEST['price']);
					}
			...
			<div class="row">
				...
				<div class="mb-3 col-sm-6">
					...
					<input type="text" class="form-control" id="name" name="name"
						placeholder="Enter Card Name" value="<?= $cardData['name'] ?? '' ?>">
				</div>
				...
				<div class="mb-3 col-sm-6">
					...
					<input type="number" class="form-control" id="price" name="price"
						placeholder="Enter Card Price" value="<?= $cardData['price'] ?? '' ?>">
				</div>
        ...
        <div class="productGallery">
            ...
            <form  method="post" enctype="multipart/form-data" class="uploadForm">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-5">
                        <input type="file" class="form-control" accept="image/*" name="productPic" required>
                    </div>
...
const cardId = "<?php echo $_REQUEST['cardID']; ?>";
...
forms.forEach(form => {
    form.addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch(`${cardId}/upload`, {
                method: 'POST',
                body: formData
            });
